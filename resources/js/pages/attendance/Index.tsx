...
function CalendarView({ month, records }: { month: string; records: AttendanceRecord[] }) {
    ...
    return (
        <div className="overflow-hidden">
            ...
            <div className="grid grid-cols-7 divide-x divide-y divide-slate-100 dark:divide-slate-800">
                {cells.map((day, i) => {
                    if (!day) return <div key={`empty-${i}`} className="h-28 bg-slate-50/50 dark:bg-slate-900/30" />;
                    ...
                    return (
                        <div
                            key={dateStr}
                            className={cn(
                                'h-28 p-1.5 flex flex-col gap-1 overflow-hidden',
                        ...
                        >
                            ...
                            {recs.slice(0, 3).map((r) => {
                                ...
                                return (
                                    <div
                                        key={r.id}
                                        className={cn('flex items-center gap-1 rounded px-1 py-0.5 text-xs truncate', sc.bg, sc.text)}
                                        title={`${r.employee?.name ?? ''} · ${r.status.replace('_', ' ')} ${r.check_in ? `· ${r.check_in}` : ''}`}
                                    >
                                        <span className={cn('h-1.5 w-1.5 rounded-full flex-shrink-0', sc.dot)} />
                                        <span className="truncate">{r.employee?.name ?? r.status}</span>
                                    </div>
                                );
                            })}
                            {recs.length > 3 && (
...
function ListView({ records, onDelete }: { records: AttendanceRecord[]; onDelete: (id: number) => void }) {
    if (records.length === 0) {
        return (
            ...
            <table className="w-full text-sm">
                <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
                        {['Date','Employee','Shift','In / Out','Worked','Status',''].map((h) => (
...
export default function AttendanceIndex({ records, employees, shifts, month, selected_employee }: Props) {
    ...
    function changeEmployee(id: string) {
        navigate({ month, ...(id ? { employee_id: id } : {}) });
    }
            ...
            <div className="max-w-7xl mx-auto space-y-5">
                ...
                <div className="flex items-center justify-between flex-wrap gap-3">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-0.5">{records.length} records · {monthLabel(month)}</p>
                    </div>
                ...
                {showForm && (
                    <Card className="border-blue-200 dark:border-blue-900 bg-blue-50/50 dark:bg-blue-950/20">
                        <CardHeader className="pb-3 flex flex-row items-center justify-between">
                            <CardTitle className="text-base font-semibold text-slate-900 dark:text-white">Record Attendance</CardTitle>
                        ...
                        <CardContent>
                            <form onSubmit={submit} className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                ...
                                <div className="col-span-2 sm:col-span-3 flex gap-3 pt-1">
                                    <Button type="submit" disabled={processing} size="sm" className="bg-blue-600 hover:bg-blue-500">Save Record</Button>
                ...
                <Card className="border-slate-200 dark:border-slate-800 overflow-hidden">
                    {view === 'calendar'
                        ...
                        : <ListView records={records} onDelete={deleteRecord} />
                    }
                </Card>
            </div>
        </AppLayout>
