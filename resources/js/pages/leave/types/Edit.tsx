...
interface LeaveType {
    id: number; name: string; code: string; days_allowed: number; is_paid: boolean;
    is_carry_forward: boolean; max_carry_forward: number; allow_half_day: boolean;
    color: string; is_active: boolean; description: string | null;
}

export default function LeaveTypeEdit({ type }: { type: LeaveType }) {
    const { data, setData, put, processing, errors } = useForm({
        name: type.name, code: type.code, days_allowed: String(type.days_allowed),
        is_paid: type.is_paid, is_carry_forward: type.is_carry_forward,
        max_carry_forward: String(type.max_carry_forward),
        allow_half_day: type.allow_half_day, color: type.color,
        is_active: type.is_active, description: type.description ?? '',
    });
    ...
    return (
        <AppLayout>
            <div className="max-w-xl mx-auto space-y-5">
                ...
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Edit — {type.name}</h1>
                    ...
                    <Card className="border-slate-200 dark:border-slate-800">
                        ...
                        <CardContent className="grid grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Input value={data.code} onChange={(e) => setData('code', e.target.value.toUpperCase())} maxLength={20} />
                                {errors.code && <p className="text-xs text-red-500">{errors.code}</p>}
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-sm font-medium text-slate-700 dark:text-slate-300">Days / Year *</Label>
